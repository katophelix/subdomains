...
document.getElementById("photo").src="http://selfie.saltydog.com/src/image-" + number + ".jpg";
...
function test4(){
	...
	$.ajax({
           type: "POST",
           url: 'http://selfie.saltydog.com/test4.php',
...
function test6(){
	...
	$.ajax({
           type: "POST",
           url: 'http://selfie.saltydog.com/test6.php',
           data: $("#webshot").serialize(), // serializes the form's elements.
           success: function(data)
           {
			 console.log(data);
...
function test2(){
	var url = 'http://selfie.saltydog.com/test2.php';
    var postData = $('#webshot').serialize();
	var message = $('textarea[name=message]').value;
	if ($('.message').val().length === 0 ){
		$('.message').val('No Message');
	...
	$("#webshot").on('click','#submit',function(e)
{
	e.preventDefault();
	mainView.router.loadPage('http://selfie.saltydog.com/selfie/dev/index.php');
    var url = 'http://selfie.saltydog.com/test3.php';
    var postData = $('#webshot').serialize();
    var formURL = url;
    $.ajax({
           type: "POST",
