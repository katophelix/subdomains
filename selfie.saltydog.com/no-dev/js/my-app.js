...
document.getElementById("photo").src="https://saltydog.com/src/image-" + number + ".jpg";
...
function test4(){
	...
	$.ajax({
           type: "POST",
           url: 'https://saltydog.com/test4.php',
...
function test6(){
	...
	$.ajax({
           type: "POST",
           url: 'https://saltydog.com/test6.php',
           data: $("#webshot").serialize(), // serializes the form's elements.
           success: function(data)
           {
			 console.log(data);
...
function test2(){
	var url = 'https://saltydog.com/test2.php';
    var postData = $('#webshot').serialize();
	var message = $('textarea[name=message]').value;
	if ($('.message').val().length === 0 ){
		$('.message').val('No Message');
